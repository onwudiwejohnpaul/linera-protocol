...
use linera_base::{crypto::secp256k1::Secp256k1SecretKey, time::Duration};
...
pub struct ChainWorkerConfig {
    ...
    pub key_pair: Option<Arc<Secp256k1SecretKey>>,
...
impl ChainWorkerConfig {
    ...
    pub fn with_key_pair(mut self, key_pair: impl Into<Option<Secp256k1SecretKey>>) -> Self {
        self.key_pair = key_pair.into().map(Arc::new);
        self
    }

    /// Gets a reference to the [`KeyPair`], if available.
    pub fn key_pair(&self) -> Option<&Secp256k1SecretKey> {
