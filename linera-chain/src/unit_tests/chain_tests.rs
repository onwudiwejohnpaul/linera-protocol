...
use linera_base::{
    crypto::{secp256k1::Secp256k1PublicKey, CryptoHash},
    data_types::{
...
fn make_open_chain_config() -> OpenChainConfig {
    let committee = Committee::make_simple(vec![Secp256k1PublicKey::test_key(1).into()]);
    OpenChainConfig {
        ownership: ChainOwnership::single(Secp256k1PublicKey::test_key(0).into()),
        admin_id: admin_id(),
        epoch: Epoch::ZERO,
...
async fn test_block_size_limit() {
    ...
    config.committees.insert(
        ...
        Committee::new(
            BTreeMap::from([(
                ValidatorName(Secp256k1PublicKey::test_key(1)),
                ValidatorState {
                    network_address: Secp256k1PublicKey::test_key(1).to_string(),
                    votes: 1,
                },
            )]),
