...
use linera_base::{
    crypto::{secp256k1::Secp256k1Signature, CryptoHash},
    data_types::Round,
    hashed::Hashed,
};
...
impl<T> GenericCertificate<T> {
    pub fn new(
        value: Hashed<T>,
        round: Round,
        mut signatures: Vec<(ValidatorName, Secp256k1Signature)>,
    ) -> Self {
    ...
    pub fn destructure(self) -> (Hashed<T>, Round, Vec<(ValidatorName, Secp256k1Signature)>) {
        (self.value, self.round, self.signatures)
    }

    pub fn signatures(&self) -> &Vec<(ValidatorName, Secp256k1Signature)> {
        &self.signatures
    }

    #[cfg(with_testing)]
    pub fn signatures_mut(&mut self) -> &mut Vec<(ValidatorName, Secp256k1Signature)> {
        &mut self.signatures
    }
    ...
    pub fn add_signature(
        &mut self,
        signature: (ValidatorName, Secp256k1Signature),
    ) -> &Vec<(ValidatorName, Secp256k1Signature)> {
        let index = self
            .signatures
