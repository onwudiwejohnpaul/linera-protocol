...
use linera_base::{
    crypto::secp256k1::Secp256k1PublicKey,
    data_types::{
...
async fn test_simple_user_operation() -> anyhow::Result<()> {
    ...
    let owner = Owner::from(Secp256k1PublicKey::test_key(0));
    let state_key = vec![];
...
async fn test_open_chain() -> anyhow::Result<()> {
    let committee = Committee::make_simple(vec![Secp256k1PublicKey::test_key(0).into()]);
    let committees = BTreeMap::from([(Epoch::ZERO, committee)]);
    let chain_key = Secp256k1PublicKey::test_key(1);
    let ownership = ChainOwnership::single(chain_key.into());
    let child_ownership = ChainOwnership::single(Secp256k1PublicKey::test_key(2).into());
...
async fn test_close_chain() -> anyhow::Result<()> {
    let committee = Committee::make_simple(vec![Secp256k1PublicKey::test_key(0).into()]);
    let committees = BTreeMap::from([(Epoch::ZERO, committee)]);
    let ownership = ChainOwnership::single(Secp256k1PublicKey::test_key(1).into());
    let state = SystemExecutionState {
        committees: committees.clone(),
        ownership: ownership.clone(),
        balance: Amount::from_tokens(5),
...
/// messages.
#[test_case(
    Some(Secp256k1PublicKey::test_key(1).into()), Some(Secp256k1PublicKey::test_key(1).into())
    => matches Ok(Ok(()));
    "works if sender is a receiving chain owner"
)]
#[test_case(
    Some(Secp256k1PublicKey::test_key(1).into()), Some(Secp256k1PublicKey::test_key(2).into())
    => matches Ok(Err(
...
#[test_case(
    Some(Secp256k1PublicKey::test_key(1).into()), None
    => matches Ok(Err(
        ExecutionError::SystemError(SystemExecutionError::UnauthenticatedTransferOwner)
    ));
...
#[test_case(
    None, Some(Secp256k1PublicKey::test_key(1).into())
    => matches Ok(Err(
        ExecutionError::SystemError(SystemExecutionError::UnauthenticatedTransferOwner)
    ));
